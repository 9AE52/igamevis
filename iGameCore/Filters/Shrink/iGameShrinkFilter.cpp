...
void ShrinkFilter::SetShrinkFactor(double factor) { m_ShrinkFactor = std::clamp(factor, 0.0, 1.0); }
...
ArrayObject::Pointer ShrinkFilter::CloneArray(ArrayObject::Pointer src, const std::vector<IGsize>& srcOfNew) {
    ...
    switch (src->GetArrayType()) {
        ...
        case IG_UnsignedCharArray:
            dst = UnsignedCharArray::New();
            break;
        case IG_ShortArray:
            dst = ShortArray::New();
            break;
        case IG_UnsignedShortArray:
            dst = UnsignedShortArray::New();
            break;
        case IG_IntArray:
            dst = IntArray::New();
            break;
        case IG_UnsignedIntArray:
            dst = UnsignedIntArray::New();
            break;
        case IG_LongLongArray:
            dst = LongLongArray::New();
            break;
        case IG_UnsignedLongLongArray:
            dst = UnsignedLongLongArray::New();
            break;
        case IG_FloatArray:
            dst = FloatArray::New();
            break;
    ...
    for (IGsize i = 0; i < newCount; i++) {
        IGsize s = srcOfNew[i];
        for (IGsize c = 0; c < dim; c++) { dst->SetValue(i * dim + c, src->GetValue(s * dim + c)); }
    }
    return dst;
}
...
bool ShrinkFilter::Execute() {
    ...
    auto shrinkCells = [&](PointSet* pointSet, IGsize count, CellArray* cells, auto getCellPointIds) -> bool {
